...
async def register_user(session: AsyncSession, user: User, full_name: str, phone: str) -> User:
    """Foydalanuvchi ariza qoldiradi — status 'pending' bo'ladi."""
    user.full_name = full_name.strip()[:128]
...
async def list_pending_users(session: AsyncSession) -> list[dict]:
    """Admin uchun: kutayotgan arizalar ro'yxati."""
...
async def set_user_status(session: AsyncSession, user_db_id: int, status: str) -> User | None:
    """Admin foydalanuvchini tasdiqlaydi yoki rad etadi."""
    if status not in ("approved", "rejected"):
        raise ValueError("Noto'g'ri status")
    u = await session.get(User, user_db_id)
    if u is None:
        return None
    u.status = status
    await session.commit()
...
async def get_user_stats(session: AsyncSession, user: User) -> dict:
    ...
    return {
        "user": {
            "telegram_id": user.telegram_id,
            "name": user.full_name or user.first_name or user.username or "Anonim",
            "photo_url": user.photo_url,
            "status": user.status,
            "phone": user.phone,
            "is_admin": user.telegram_id == settings.ADMIN_ID,
        },
